...
# Handle GradientAccumulationPlugin import for different versions
try:
    # For older versions of transformers
    from transformers.trainer import GradientAccumulationPlugin
except ImportError:
    try:
        # For newer versions, it might be in accelerate
        from accelerate.utils import GradientAccumulationPlugin
    except ImportError:
        # If not available anywhere, create a dummy class
        class GradientAccumulationPlugin:
            def __init__(self, **kwargs):
                self.num_steps = kwargs.get("num_steps", 1)
                self.sync_with_dataloader = kwargs.get("sync_with_dataloader", False)
...
from model.llava_utils.utils import rank0_print
...
class LLaVATrainer(Trainer):
    ...
    def create_accelerator_and_postprocess(self):
        # Handle different versions of accelerate/transformers
        try:
            # For newer versions (transformers >= 4.38, accelerate >= 1.3)
            grad_acc_kwargs = {"num_steps": self.args.gradient_accumulation_steps}
            grad_acc_kwargs["sync_with_dataloader"] = False
            
            # Try to import GradientAccumulationPlugin
            try:
                from accelerate.utils import GradientAccumulationPlugin
                ...
                try:
                    ...
                    # If GradientAccumulationPlugin doesn't exist, don't use it
            ...
            if gradient_accumulation_plugin is not None:
                ...
                # For newest versions where gradient accumulation is handled differently
                self.accelerator = Accelerator(
                    #dispatch_batches=self.args.dispatch_batches, 
                    split_batches=self.args.split_batches, 
                    deepspeed_plugin=self.args.deepspeed_plugin,
            ...
            # Fallback to basic accelerator initialization
            rank0_print(f"Using fallback accelerator initialization: {e}")
            self.accelerator = Accelerator(
                #dispatch_batches=self.args.dispatch_batches, 
                split_batches=self.args.split_batches, 
                deepspeed_plugin=self.args.deepspeed_plugin
