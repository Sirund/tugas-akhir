...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.model_class_name is not None:
        ...
        if "qwen" in model_args.model_name_or_path.lower() or "robobrain" in model_args.model_name_or_path.lower():
...
def train(attn_implementation=None):
    ...
    try:
        # Existing logic that uses tokenizer
        ...
        rank0_print(f"Tokenizer initialized: {e}")
        ...
        rank0_print(f"Tokenizer initialized successfully")
        
        # Now retry the logic with the initialized tokenizer
        if model_args.version == "v0":
            if tokenizer.pad_token is None:
                smart_tokenizer_and_embedding_resize(
                    special_tokens_dict=dict(pad_token="[PAD]"),
